        ...
        <div className="flex flex-col h-screen bg-white dark:bg-gray-900">
            ...
            <header className="p-6 pb-2 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center text-kisaan-600 mb-2">
                    <Leaf className="mr-2" />
                    <span className="font-bold text-lg text-gray-900 dark:text-white">Kisaan Mitra</span>
                </div>
                {!isUpdateMode && (
                    <div className="flex items-center mb-4">
                        <div className={`flex-1 h-1.5 rounded-l-full ${step >= 1 ? 'bg-kisaan-500' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                        <div className={`flex-1 h-1.5 rounded-r-full ml-1 ${step >= 2 ? 'bg-kisaan-500' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                    </div>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col">
                ...
                {step === 1 && (
                    <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Main Crop</label>
                        <input
                            type="text"
                            value={crop}
                            onChange={(e) => setCrop(e.target.value)}
                            className="block w-full rounded-xl border-gray-300 shadow-sm focus:border-kisaan-500 p-4 bg-gray-50 dark:bg-gray-800 text-lg"
                            placeholder="e.g. Cotton, Wheat, Sugarcane"
                            autoFocus
                        />
                        <div className="mt-8">
                            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Popular Options</h3>
                                ...
                                {['Cotton', 'Wheat', 'Soybean', 'Rice', 'Sugarcane', 'Onion', 'Tomato'].map(preset => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-full border text-sm transition-colors ${
                                            crop === preset
                                                ? 'bg-kisaan-50 border-kisaan-500 text-kisaan-700 font-medium'
                                                : 'bg-white border-gray-200 text-gray-600 hover:border-kisaan-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300'
                                        }`}
                                    >
                                        {preset}
                                    </button>
                        ...
                        <div className="flex gap-2 mb-4">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="block w-full rounded-xl border-gray-200 shadow-sm pl-10 p-3 bg-gray-50 dark:bg-gray-800 text-sm"
                                    placeholder="Search city or district..."
                                />
                            </div>
                            <button onClick={handleSearchLocation} className="px-4 bg-kisaan-600 text-white rounded-xl text-sm font-medium hover:bg-kisaan-700">
                                Search
                            </button>
                            <button onClick={handleAutoDetect} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100" title="Auto-detect GPS">
                                <Navigation size={20} />
                            </button>
                        </div>
            ...
            <div className="p-6 border-t border-gray-100 dark:border-gray-800">
                {step === 1 ? (
                    ...
                    <div className="flex gap-3">
                        {!isUpdateMode && (
                            <button onClick={() => setStep(1)} className="px-6 py-4 rounded-xl font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300">
                                Back
                            </button>
                        )}
                        <button
                        ...
                        >
                            {loading ? 'Saving...' : isUpdateMode ? 'Save Location' : 'Finish Setup'}
